...
export class DarkModeToggle extends HTMLElement {
  ...
  _initializeDOM() {
    ...
    // Use link references captured before FOUC script changed their media
    // attributes (stored in window.__pcsLinks by the anti-FOUC inline script).
    // Fall back to querying by media attribute when __pcsLinks is absent.
    const pcs = window.__pcsLinks;
    if (pcs) {
      this._darkCSS  = pcs.dark;
      this._lightCSS = pcs.light;
    } else {
